...
- [X] T053 **Revoke the Gemini API key at the provider.** It was never committed (`Config/Secrets.xcconfig` is git-ignored and untracked) but it has shipped inside every build made so far, and a key inside a distributed binary is extractable. Deleting it from the project does not stop it working. **Cerrada por decisión del maintainer (2026-09-01) sin revocar: la app nunca se distribuyó** — ninguna build salió del equipo del maintainer, así que la key incrustada nunca llegó a un dispositivo ajeno. La key sigue siendo válida en el proveedor; si alguna build anterior a esta feature llegara a distribuirse, esta tarea se reabre
- [X] T054 Run `/speckit-constitution` to amend the Product Overview and External Services sections: the app's single external service is now the Kalorias backend, and the app holds no provider credential. **Out of scope for this feature's code** — it is a governed document with its own version history (plan.md Complexity Tracking)
